...
const logger = {
  fatal(msg) {
    if (PLAYGROUND.LOGLEVEL > 0) {
      console.error(`%c☠ ${msg}`, `color:crimson;font-weight:bold;font-size:2rem;`);
    }
  },
  error(msg) {
    if (PLAYGROUND.LOGLEVEL > 1) {
      console.error(`💣 ${msg}`);
    }
  },
  warn(msg) {
    if (PLAYGROUND.LOGLEVEL > 2) {
      console.warn(`⚠ ${msg}`);
    }
  },
  info(msg) {
    if (PLAYGROUND.LOGLEVEL > 3) {
      console.info(`ℹ %c${msg}`, `color:#061615;background:#3acabb;`);
    }
  },
  debug(msg) {
    if (PLAYGROUND.LOGLEVEL > 4) {
      console.debug(`🐞 ${msg}`);
    }
  },
  trace(msg) {
    if (PLAYGROUND.LOGLEVEL > 5) {
      console.trace(msg);
    }
  },
};
...
export default {
  ...
  broadcast(event = {}, type = 'message', element = document, obj = {}) {
    return element.dispatchEvent(
      new CustomEvent(type, { detail: Object.assign(obj, { target: event.target }), bubbles: true })
    );
  },
